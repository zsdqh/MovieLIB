/**
 * Страница /filter: пагинация через JSON, параметры из текущего URL.
 */
(function () {
  ...
  function filterUrlWithPage(nextPage) {
  ...
  if (!root || root.dataset.pageKind !== "filter") return;
  ...
  function applyState(data) {
    ...
    const newUrl = "/filter?" + params.toString();
  ...
  async function loadPage(direction) {
    ...
    const url = filterUrlWithPage(nextPage);
    try {
