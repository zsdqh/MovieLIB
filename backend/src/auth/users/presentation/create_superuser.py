...
from backend.src.auth.users.domain.dtos import UserRegisterDTO
...
async def create_superuser(
    ...
    async with uow:
        new_user = await UserRegisterUseCase(uow, pwd_hasher)(
...
def main() -> None:
    ...
    asyncio.run(
        create_superuser(args.username, args.email, args.password, uow, pwd_hasher)
    )
